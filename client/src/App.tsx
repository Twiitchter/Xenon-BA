...
function AppContent() {
  ...
  const [adminViewMode, setAdminViewMode] = useState<'front-end' | 'back-end'>('back-end');

  useEffect(() => {
    checkAuth();
    // Load admin view preference from localStorage
    const savedMode = localStorage.getItem('adminViewMode') as 'front-end' | 'back-end';
    if (savedMode) {
    ...
    <div className="app">
      ...
      <div className={isAuthenticated ? 'main-content' : ''}>
        <Routes>
          ...
          <Route
            path="/my-requests"
            element={isAuthenticated ? <MyRequests /> : <Navigate to="/login" replace />}
          />
          <Route
            path="/requests"
            element={
