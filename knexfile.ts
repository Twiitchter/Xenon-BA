...
function buildConfig(dialect: DbDialect): Knex.Config {
  ...
  const portDefaults: Record<DbDialect, number> = { pg: 5432, mssql: 1433, mysql2: 3306 };
  ...
  const connection =
    dialect === 'mssql'
      ...
      : { host: common.host, port, database: common.database, user: common.user, password: common.password };
